...
REQUIRED_CHANNELS = [
    {"username": "@RgxNumberChanel", "name": "📢 Tech Channel", "url": "https://t.me/RgxNumberChanel"},
    {"username": "@RgxOtp", "name": "👥 OTP Group", "url": "https://t.me/RgxOtp"},
]
...
def join_required_keyboard(not_joined_channels: list) -> InlineKeyboardMarkup:
    """Create keyboard with only channels that user hasn't joined yet"""
    rows = []
...
async def check_joined_channels_detailed(bot, user_id):
    """Check which channels user has joined and return detailed results"""
    not_joined = []
    joined = []
    ...
    for channel in REQUIRED_CHANNELS:
        try:
            ...
            print(f"Join check error for user {user_id} on {channel['username']}: {e}")
            not_joined.append(channel)
    
    return joined, not_joined

async def check_joined_channels(bot, user_id):
    """Simple check if user joined all channels"""
    _, not_joined = await check_joined_channels_detailed(bot, user_id)
...
async def send_join_required(bot, user_id):
    """Send join required message showing only unjoined channels"""
    _, not_joined = await check_joined_channels_detailed(bot, user_id)
    
    if not not_joined:
        return  # All joined, no need to send message
    
    # Build message listing only not-joined channels
    channels_text = ""
    for i, channel in enumerate(not_joined, 1):
        channels_text += f"{i}️⃣ {channel['name']}\n"
    
    message = (
        "⚠️ *Access Required*\n\n"
        "You haven't joined the following:\n\n"
        f"{channels_text}\n"
        "✅ After joining ALL, click *I've Joined All — Verify* below."
    )
    
    await bot.send_message(
        user_id,
        message,
        reply_markup=join_required_keyboard(not_joined),
        parse_mode='Markdown',
    )
...
async def check_joined_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    joined, not_joined = await check_joined_channels_detailed(context.bot, user_id)
    ...
    if len(not_joined) == 0:
        ...
        # Some channels not joined - show which ones
        channels_text = ""
        for i, channel in enumerate(not_joined, 1):
            channels_text += f"{i}️⃣ {channel['name']}\n"
...
def main():
    ...
    print("✅ Join Check Active with Detailed Channel Tracking")
    ...
    print("✅ FIXED: Join check now shows only unjoined channels")
